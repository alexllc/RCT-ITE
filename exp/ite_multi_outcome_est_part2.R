...
trial_ls <- trial_ls[9:length(trial_ls)]
trial_ls <- "NCT00041119length"
    ...
    for (trial in trial_ls) {
        ...
        for (outcome in outcome_list) {
            ...
            tau <- perform_rstack(Y = Y_rmvd, X = X_rmvd, W = W_rmvd, trial_name = trial, outcome = outcome, imp_type_Y = imp_type_Y)
